...
echo X.X.X.X	MYSQL SERVER HOSTNAME >> /etc/hosts
...
sleep 3
	...
	cd /var/www/html/snorby/
		...
		sed -i 's,Enter Password Here,PUT YOUR PASSWORD HERE,g' /var/www/html/snorby/config/database.yml
		sed -i 's,localhost,HOSTNAME OR IP ADDRESS OF YOUR MYSQL SERVER,g' /var/www/html/snorby/config/database.yml
...
# SNORBY VIRTUALHOSTS
# Change the parameters as you wish. e.g: admin@example.com >>>> administrator@your.domain
...
# Optional: There is no need to reboot. It is only to assure all functions will be executed after an OS restart.
